...
public class BluetoothClient extends Activity {
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            if (BluetoothDevice.ACTION_FOUND.equals(action)) {
                ...
                if(device!= null) {
                    if(device.getName().contentEquals("Nexus")) {
                        btArray.add(device.getName() + "\n" + device.getAddress());
                        Log.i(TAG, "adding a nexus in: broadcastreceiver");
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        registerReceiver(loggingReceiver, new IntentFilter("log"));
        ctx = this;
                ...
                switch (msg.what) {
                    ...
                    case MESSAGE_READ:
                        ...
                        if (readMessage.length() > 0) {
                            ...
                            newMessage.putExtra("message", readMessage);
        ...
        if (pairedDevices.size() > 0) {
            ...
            for (BluetoothDevice device : pairedDevices) {
                ...
                if(device.getName().contains("Nexus")) {
                    btArray.add(device.getName() + "\n" + device.getAddress());
                    btArray.notifyDataSetChanged();
                    Log.i(TAG, "adding nexus in: setup");
                } else {
                    btArray.add(device.getName() + "\n" + device.getAddress());
                    btArray.notifyDataSetChanged();
                }
    ...
    private class ConnectedThread extends Thread {
        ...
        public ConnectedThread(BluetoothSocket socket) {
            ...
            mmInStream = tmpIn;
            mmOutStream = tmpOut;
            write("connected to bluetooth".getBytes());
        }
